...
class SpotDetail extends Component {
  ...
  getWeeklySpitData() {
    fetch(
      `http://api.spitcast.com/api/spot/forecast/${
        this.props.router.location.spitID
      }/?dcat=week`
    )
      .then(response => response.json())
      .then(json => {
        ...
        <div className="tides padding">
          ...
          <div className="frames">
            <iframe
              ...
              width="100%"
              height="650"
              src={`https://embed.windytv.com/embed2.html?lat=${
                this.state.forecast.lat
              }&lon=${
                this.state.forecast.lon
              }&zoom=13&level=surface&overlay=wind&menu=&message=&marker=&forecast=12&calendar=now&location=coordinates&type=map&actualGrid=&metricWind=kt&metricTemp=%C2%B0C`}
              frameBorder="0"
            />
            <iframe
              className="frames"
              ...
              src={`https://www.google.com/maps/embed/v1/place?key=${yungKeys}&q=${
                this.state.forecast.lat
              }, ${this.state.forecast.lon}&maptype=satellite`}
            />
          </div>
        </div>
      </div>
